...
def data_loader(retain: List[str], sex: bool = False) -> Union[pd.DataFrame, Tuple[pd.DataFrame, pd.DataFrame]]:
    ...
    data = pd.read_csv(
        ...
        delimiter=";",
        dtype=dtype_dict,
        usecols=retain + ["agecat", "sex"] if sex else retain + ["agecat"],
        decimal=",",
...
def run_fci(
    ...
    g, pag = fci(data, test_map.get(citest), alpha=sign_level, verbose=True)

    filename = f"FCI_{citest}_{sign_level}_{subscript}.png".replace(".", "_")
    output_path = os.path.join(output_dir, filename)

    # Save graph visualization
    GraphUtils.to_pydot(g, labels=feature_names).savefig(
...
def iterator_over_fci(retain):
    ...
    data_list  = list(map(data_prep, [data_fem, data_men, data]))
